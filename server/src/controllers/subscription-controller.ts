...
export const createSubscription: RequestHandler = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (existingSubscription) {
      ...
      logger.info(`New subscription created for user ${userId}`, { subscription_id: newSubscription.subscription_id });
      res.status(201).json({
...
export const deleteSubscription: RequestHandler = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!existingSubscription) {
      logger.warn(`Subscription not found for user ${userId} with endpoint ${endpoint}`);
    ...
    logger.info(`Subscription deleted for user ${userId}`, { subscription_id: existingSubscription.subscription_id });
